    ...
    function Proses(){
        if(getOne2("select count(pasien.no_rkm_medis) from pasien inner join reg_periksa on reg_periksa.no_rkm_medis=pasien.no_rkm_medis inner join kamar_inap on reg_periksa.no_rawat=kamar_inap.no_rawat where kamar_inap.stts_pulang='-' and pasien.no_rkm_medis='".encrypt_decrypt($_SESSION["ses_pasien"],"d")."'")>0){
            ...
            if (isset($kd_dokter)) {
                ...
                if($interval>0 && $interval<8){ //edit-novan cara membatasi booking hanya bisa h-1 s/d h-7 tambahkan && $interval<8
                    ...
                    if(($kuota>0)&&($daftar>=$kuota)){
                        ...
                        if(!isset($_SESSION["penjab"])){
                            ...
                            $querypenjab  = bukaquery("SELECT penjab.kd_pj,penjab.png_jawab FROM pasien INNER JOIN penjab ON penjab.kd_pj=pasien.kd_pj WHERE pasien.no_rkm_medis='".encrypt_decrypt($_SESSION["ses_pasien"],"d")."' AND pasien.kd_pj<>'BPJ'"); //novan custom pilih berdasarkan jenis bayar
                    ...
                    echo "<div class='block-header'>
                          ...
                          <div class='row clearfix'>
                             <div class='col-lg-12 col-md-12 col-sm-12 col-xs-12'>
                                <div class='card'>
                                    <div class='body'>
                                        <center>Mohon maaf 🙏🏻, Batas booking hanya 1 hari s/d 7 hari sebelum periksa</center><br/>
                                        <center><a href='index.php?act=BookingRegistrasi&hal=Booking' class='btn btn-danger waves-effect'>Kembali</a></center>
                                    </div>
                                </div>
                             </div>
        ...
        if ((!empty($penjab))&&(!empty($kd_poli))&&(!empty($tanggal))&&(!empty($kd_dokter))) {
            ...
            switch (URUTNOREG) {
                ...
                case "poli-jkn" : 
                    ...
                    break;                
                case "dokter-jkn" : 
                			if(getOne("select ifnull(MAX(CONVERT(no_reg,signed)),0)+1 from booking_registrasi where kd_dokter='$kd_dokter' and tanggal_periksa='$tanggal'")>=
                			getOne("select ifnull(MAX(CONVERT(no_reg,signed)),0)+1 from reg_periksa where kd_dokter='$kd_dokter' and tgl_registrasi='$tanggal'")){
            		...
            		$max    = getOne("select ifnull(MAX(CONVERT(no_reg,signed)),0)+1 from reg_periksa where kd_dokter='$kd_dokter' and tgl_registrasi='$tanggal'");
            		$nourut = sprintf("%03s", $max);
        		}
                    break;
                case "dokter" : 
                  ...
                  <div class='row clearfix'>
                     <div class='col-lg-12 col-md-12 col-sm-12 col-xs-12'>
                        <div class='card'>
                            <div class='body'>
                                <center>
                                Mohon Maaf 🙏, pasien BPJS silahkan mendaftar melalui Mobile JKN
                                <br/>
                                </center><br/>
                                <center><a href='index.php?act=BookingRegistrasi&hal=Booking' class='btn btn-danger waves-effect'>Kembali</a></center>
                            </div>
