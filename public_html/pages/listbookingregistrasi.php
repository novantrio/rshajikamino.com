                    ...
                    <div role="tabpanel" class="tab-pane fade in active" id="home">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                ...
                                <tbody>
                                <?php 
                                    $queryjadwal = bukaquery("Select dokter.nm_dokter,poliklinik.nm_poli,jadwal.jam_mulai,jadwal.jam_selesai,poliklinik.kd_poli,jadwal.kuota,dokter.kd_dokter from jadwal inner join dokter on dokter.kd_dokter=jadwal.kd_dokter inner join poliklinik on jadwal.kd_poli=poliklinik.kd_poli where jadwal.kd_poli<>'UMUM' AND jadwal.hari_kerja like '%".konversiHari($hari)."%'");
                                    while($rsqueryjadwal = mysqli_fetch_array($queryjadwal)) {
                                        $daftar     = getOne("SELECT (select count(no_rkm_medis) from booking_registrasi where 
															tanggal_periksa='$thncari-$blncari-$tglcari' and kd_dokter='".$rsqueryjadwal["kd_dokter"]."' and kd_poli='".$rsqueryjadwal["kd_poli"]."') + 
															(select count(no_rkm_medis) from reg_periksa where 
															tgl_registrasi='$thncari-$blncari-$tglcari' and kd_dokter='".$rsqueryjadwal["kd_dokter"]."' and kd_poli='".$rsqueryjadwal["kd_poli"]."')"); 
                                        $terdaftar  = getOne("SELECT (select count(no_rkm_medis) from booking_registrasi where 
															tanggal_periksa='$thncari-$blncari-$tglcari' and kd_dokter='".$rsqueryjadwal["kd_dokter"]."' and kd_poli='".$rsqueryjadwal["kd_poli"]."' and no_rkm_medis='".encrypt_decrypt($_SESSION["ses_pasien"],"d")."')" );
